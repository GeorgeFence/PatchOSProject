...
namespace PatchOS.Files
{
    public static class CPU
    {
        ...
        public static void Clock()
        {
            if (!IsHalted)
            {
                ...
                if (ProgCtr >= Memory.Size)
                {
                    Runner.Stop();
                    ProgCtr = 0;
                    CLI.WriteLine("Program counter left memory space", ColorFile.DarkYellow);
                }
        ...
        public static void DrawDebug()
        {
            ...
            if (regs.Length > 0) { TextGraphics.DrawString(0, CLI.Height - 1, regs, ColorFile.Cyan, CLI.BackColor); }

            // draw program counter
            Console.SetCursorPosition(48, CLI.Height - 1);
            TextGraphics.DrawString(48, CLI.Height - 1, "PC=" + DataUtils.IntToHex(ProgCtr), ColorFile.Yellow, CLI.BackColor);

            // draw i register
            Console.SetCursorPosition(58, CLI.Height - 1);
            TextGraphics.DrawString(58, CLI.Height - 1, "I=" + DataUtils.IntToHex(I), ColorFile.Green, CLI.BackColor);

            // draw w register
            Console.SetCursorPosition(67, CLI.Height - 1);
            TextGraphics.DrawString(67, CLI.Height - 1, "W=" + DataUtils.IntToHex(W), ColorFile.Magenta, CLI.BackColor);

            // draw waiting
            Console.SetCursorPosition(72, CLI.Height - 1);
            Console.WriteLine( "WAIT=" + (IsWaiting ? "1" : "0"), ColorFile.DarkRed, CLI.BackColor);
        }
    }
}
