...
namespace PatchOS.Files.Drivers.GUI
{
    internal class Taskbar : Process.Process
    {
        ...
        public static void DrawTaskBar()
        {
            ...
            for (int i = 0; i < ProcessManager.running.Count; i++)
            {
                if (ProcessManager.running[i].name.EndsWith(".win"))
                {
                    for (int j = 0; j < WindowManager.Windows.Count; j++)
                    {
                        if (WindowManager.Windows[j].Title == ProcessManager.running[i].name.Replace(".win", ""))
                        {
                            ...
                            Kernel.Canvas.DrawImageAlpha(WindowManager.Windows[j].Icon, X, (int)(Kernel.Canvas.Mode.Height - 40));
                            ...
                            {
                                Kernel.Canvas.DrawFilledRectangle(System.Drawing.Color.FromArgb(15, 15, 15), (X + 13) - ((WindowManager.Windows[j].Title.Length * 8) / 2) - 2, (int)(Kernel.Canvas.Mode.Height - 55), (WindowManager.Windows[j].Title.Length * 8) + 4, 18);
                                ASC16.DrawACSIIString(Kernel.Canvas, WindowManager.Windows[j].Title, System.Drawing.Color.White, (uint)(X + 13) - (uint)((WindowManager.Windows[j].Title.Length * 8) / 2), (uint)(Kernel.Canvas.Mode.Height - 53));
                            }
