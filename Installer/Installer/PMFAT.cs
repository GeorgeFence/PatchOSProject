...
namespace Installer
{
    public static class PMFAT
    {
        ...
        public static void Format(String index, String filesystem, bool quick)
        {
            ...
            {
                if (filesystem != "FAT") {
                    Console.WriteLine("Filesystem " + filesystem + " is not supported!", Color.Red);
                }
            ...
            {
                Console.WriteLine("Error while formating disc. More info: " + ex.ToString());
            }
        ...
        {
            bool value = false;
            if (FileExists(name)) { value = false; }
            else { Sys.FileSystem.VFS.VFSManager.CreateFile(name); value = true; }
            return value;
        }
        ...
        public static bool DeleteFolder(string path)
        {
            if (FolderExists(path))
            {
                try
                {
                    Console.WriteLine("Attempting to delete \"" + path + "\"", Color.Yellow);
                ...
                {
                    Console.Write("[INTERNAL] ", Color.Red); Console.WriteLine(ex.Message, Color.White);
                    return false;
                }
            }
            else { return false; }
        ...
        public static Cosmos.System.FileSystem.Listing.DirectoryEntry GetFileInfo(string file)
        {
            if (FileExists(file))
            {
                ...
                {
                    Console.WriteLine("Error occured when trying to retrieve file info", Color.Red);
                    Console.Write("[INTERNAL] ", Color.Red); Console.WriteLine(ex.Message, Color.White);
                    return null;
                }
            }
            ...
            {
                Console.WriteLine("Could not locate file \"" + file + "\"", Color.Red);
                return null;
            }
        }
        ...
        public static bool CopyFile(string src, string dest)
        {
            ...
            {
                Console.WriteLine("Error occured when trying to copy file", Color.Red);
                Console.Write("[INTERNAL] ", Color.Red); Console.WriteLine(ex.Message, Color.White);
                return false;
            }
        }
